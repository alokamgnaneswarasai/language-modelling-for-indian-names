...
Original file is located at
    https://colab.research.google.com/drive/1Rn0Wr_DUz1iit5hvKlooDL15QWkDFfEy
...
def get_trigram_counts(corpus):
    ...
    for name in corpus:
      ...
      for i in range(len(name)-2):
        current_char=name[i]
        next_char=name[i+1]
        next_next_char=name[i+2]

        trigram_counts[current_char][next_char][next_next_char]+=1
...
class UnigramModel(NGramLanguageModel):
    ...
    def get_next_char_probabilities(self):
        ...
        vocabulary = vocab.get_stoi().keys()

        for char in vocabulary:

            if char in self.unigram_counts:
                next_char_probabilities[char] = self.unigram_counts[char] / total_count

            else:
                next_char_probabilities[char] = 0
    ...
    def get_name_log_probability(self, name):
        ...
        for ch in name:
          if(next_char_probabilities[ch]!=0):
            log_prob+=math.log(next_char_probabilities[ch])
    ...
    def get_perplexity(self, text):
        ...
        N=0
        for name in text:
          sum_log_probs+=self.get_name_log_probability(name)
          N+=len(name)
    ...
    def generate_names(self, k, n=MAX_NAME_LENGTH, prefix=None):
        ...
        for _ in range(k):
            ...
            while len(name) < n:
                next_char_probabilities = self.get_next_char_probabilities()
                next_char = random.choices(list(next_char_probabilities.keys()), weights=list(next_char_probabilities.values()))[0]
                # next_char = max(next_char_probabilities, key=next_char_probabilities.get)
                if(next_char==START):
                  continue
                if next_char == END:
                    break
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        # BEGIN CODE
        next_char_probabilities = self.get_next_char_probabilities()
        most_likely_chars = sorted(next_char_probabilities.keys(), key=lambda x: next_char_probabilities[x], reverse=True)[:k]
...
class SmoothedUnigramModel(UnigramModel):
    ...
    def get_next_char_probabilities(self):
        ...
        Returns:
            key: char, value: smoothed probability
        """
        ...
        for char in vocabulary:
            ...
            if char in self.unigram_counts:
                smoothed_probabilities[char] = (self.unigram_counts[char] + 1)/ (total_tokens + vocab_size)

            else:
                smoothed_probabilities[char] = 0
...
class BigramModel(NGramLanguageModel):
    ...
    def get_next_char_probabilities(self):
        ...
        next_char_probabilities = {}
        for row_key,row_value in self.bigram_counts.items():

          if row_key not in next_char_probabilities:
            next_char_probabilities[row_key] = {}
          for col_key,col_value in  row_value.items():

            next_char_probabilities[row_key][col_key]= col_value/self.unigram_counts[row_key]

        next_char_probabilities['</s>']['</s>']=1
    ...
    def get_name_log_probability(self, name):
        ...
        log_prob =0.0
        for i in range(len(name)-1):
          if(prob.get(name[i],{}).get(name[i+1],0)!=0):
            log_prob += math.log(prob.get(name[i],{}).get(name[i+1],0))
        return log_prob
    ...
    def generate_names(self, k, n=MAX_NAME_LENGTH, prefix=None):
        ...
        for _ in range(k):
            ...
            while len(name) < n:
                ...
                current_char = name[-1] if name else '<s>'
                chars, probabilities = zip(*next_char_probabilities.get(current_char, {}).items())
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        # BEGIN CODE
        prev_char = sequence[-1] if sequence else '<s>'

        next_char_probabilities = self.get_next_char_probabilities()[prev_char]
        most_likely_chars = sorted(next_char_probabilities.keys(), key=lambda x: next_char_probabilities.get(x, 0), reverse=True)[:k]
    ...
    def get_next_char_probabilities(self):
      ...
      for row_key,row_value in self.bigram_counts.items():
        ...
        for col_key,col_value in  row_value.items():
          ...
          if(row_key=='</s>'):
            next_char_probabilities[row_key][col_key]=1/vocab_size
          else :

            next_char_probabilities[row_key][col_key]= (col_value+self.k)/(self.unigram_counts[row_key]+(self.k*vocab_size))

      # print(next_char_probabilities)

      # END CODE

      # next_char_probabilities['</s>']['</s>']=1

      # for i in next_char_probabilities['</s>']:
      #   print(i,next_char_probabilities['</s>'][i])
      return next_char_probabilities
...
class InterpolationSmoothedBigramModel(BigramModel):
    ...
    def get_next_char_probabilities(self):
      ...
      bigram_probabilities = super().get_next_char_probabilities()
      smoothed_unigram_probabilities = self.smoothed_unigram_model.get_next_char_probabilities()
      interpolated_probabilities = {}

      for prev_char, next_char_prob in bigram_probabilities.items():
          interpolated_probabilities[prev_char] = {}
          for next_char, bigram_prob in next_char_prob.items():
              unigram_prob = smoothed_unigram_probabilities[next_char]
              interpolated_probabilities[prev_char][next_char] = (self.lambda_1 * bigram_prob) + (self.lambda_2 * unigram_prob)
...
# BEGIN CODE
TRIGRAM_LAMBDAS = (0.7, 0.2, 0.1) # lambdas for interpolation smoothing in trigram models
...
class TrigramModel(NGramLanguageModel):
    ...
    def get_next_char_probabilities(self):
        ...
        for c1, c1_counts in self.trigram_counts.items():

            if c1 not in trigram_probs:
                trigram_probs[c1] = {}


            for c2, c2_counts in c1_counts.items():

                if c2 not in trigram_probs[c1]:
                    trigram_probs[c1][c2] = {}


                for c3, c3_count in c2_counts.items():
                    if(self.bigram_counts[c1][c2]==0):
                      trigram_probs[c1][c2][c3] =0
                    else:
                      trigram_probs[c1][c2][c3] = c3_count / self.bigram_counts[c1][c2]

                if(sum(trigram_probs[c1][c2].values())==0):
                   for c3, c3_count in c2_counts.items():
                    trigram_probs[c1][c2][c3] = 1 / vocab_size

        interpolated_trigram_probabilities = {}

        # Iterate over the trigram counts
        for c1, c1_counts in self.trigram_counts.items():
            # If c1 is not in the interpolated_trigram_probabilities dictionary, add it
            if c1 not in interpolated_trigram_probabilities:
                interpolated_trigram_probabilities[c1] = {}

            # Iterate over the next character counts given c1
            for c2, c2_counts in c1_counts.items():
                # If c2 is not in the interpolated_trigram_probabilities dictionary for c1, add it
                if c2 not in interpolated_trigram_probabilities[c1]:
                    interpolated_trigram_probabilities[c1][c2] = {}

                # Iterate over the counts of the next characters given c1 and c2
                for c3, c3_count in c2_counts.items():
                    trigram_prob = trigram_probs[c1][c2][c3]
                    bigram_prob = self.bigram_probs[c2][c3]
                    unigram_prob = self.unigram_probs[c3]
                    interpolated_prob = (self.lambda_1 * trigram_prob) + (self.lambda_2 * bigram_prob) + (self.lambda_3 * unigram_prob)
                    interpolated_trigram_probabilities[c1][c2][c3] = interpolated_prob

        # END CODE

        return interpolated_trigram_probabilities
    ...
    def get_name_log_probability(self, name):
        ...
        log_prob =0.0
        for i in range(len(name)-2):
          log_prob += math.log(prob.get(name[i],{}).get(name[i+1],{}).get(name[i+2],1))
    ...
    def generate_names(self, k, n=MAX_NAME_LENGTH, prefix=None):
        ...
        for _ in range(k):
            ...
            while len(name) < n:
                ...
                prev_char = name[-2] if len(name) > 1 else '<s>'


                chars, probabilities = zip(*next_char_probabilities.get(prev_char, {}).get(current_char, {}).items())
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        current_char = sequence[-1] if sequence else '<s>'
        prev_char = sequence[-2] if len(sequence) >= 2 else '<s>'


        next_char_probabilities = self.get_next_char_probabilities()[prev_char][current_char]
...
class FNN_LM(nn.Module):
    ...
    def forward(self, chars):
        ...
        x1= F.tanh(self.linear1(flattened))
        x1= self.linear2(x1)
        x2=self.linear3(flattened)
        x= x1+x2
        return x
         # END CODE
...
class NeuralNGramTrainer:
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
          ...
          # print(f"{epoch} loss is {train_loss}")
    ...
    def get_next_char_probabilities(self , prefix=None):
        ...
        prefix_tensor = torch.tensor([[self.vocab[token] for token in prefix]], dtype=torch.long)
        ...
        next_char_probabilities = {char: probabilities[idx].item() for char, idx in self.vocab.get_stoi().items()}
    ...
    def generate_names(self, k, n=MAX_NAME_LENGTH, prefix=None):
        ...
        if prefix is None:
            prefix = ['<s>'] * (self.ngram - 1)
        while(len(prefix)<self.ngram-1):
          prefix.insert(0, '<s>')

        generated_names = []

        with torch.no_grad():
            while len(generated_names) < k:

                current_sequence = prefix[:]

                # print(f"current sequence is {current_sequence}")
                # Generate tokens until reaching maximum length or encountering an end token
                while len(current_sequence) < n:
                    next_char_probabilities = self.get_next_char_probabilities(current_sequence[-(self.ngram - 1):])
                ...
                generated_name = ''.join(current_sequence)
        ...
        with torch.no_grad():
            for name in text:
                ...
                for i in range(len(name) - self.ngram + 1):
                    prefix = name[i:i + self.ngram - 1]
                    target = name[i + self.ngram - 1]
                    output_probs = self.get_next_char_probabilities(prefix)
                    target_prob = output_probs[target]
                    if target_prob == 0:
                        target_prob = 1e-10

                    name_log_prob += np.log(target_prob)
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        most_likely_chars = sorted(next_char_probabilities.keys(), key=lambda char: next_char_probabilities[char], reverse=True)[:k]
...
model_dir = 'ALOKAMGNANESWARASAI_22582/fnn'
...
folder = 'ALOKAMGNANESWARASAI_22582/fnn'
...
class RNNTrainer:
    ...
    def validate_epoch(self):
      ...
      epoch_loss /= len(self.valid_dataloader)
    ...
    def get_next_char_probabilities(self):
        ...
        self.model.eval()
        sequence = ["<s>"]
        input_sequence = torch.tensor([self.vocab[char] for char in sequence]).unsqueeze(0)
        with torch.no_grad():
            output = self.model(input_sequence)

        probabilities = F.softmax(output[:, -1, :], dim=-1).squeeze().cpu().numpy()
    ...
    def generate_names(self, k, n, prefix=None):
        ...
        count=0
        while count<k:
            sequence = prefix[:] if prefix else ["<s>"]
            for _ in range(n):

                input = torch.tensor([self.vocab[char] for char in sequence]).unsqueeze(0)

                with torch.no_grad():
                    output = self.model(input)


                predicted_char_idx = torch.multinomial(torch.softmax(output[:, -1, :], dim=-1), 1).item()


                predicted_char = None

                for char, idx in self.vocab.get_stoi().items():
                    if idx == predicted_char_idx:
                        predicted_char = char
                        break
    ...
    def get_perplexity(self, text):
        ...
        with torch.no_grad():
            for sequence in text:
                ...
                input_sequence = torch.tensor([self.vocab[char] for char in sequence[:-1]]).unsqueeze(0)
                target_sequence = torch.tensor([self.vocab[char] for char in sequence[1:]]).unsqueeze(0)


                output = self.model(input_sequence)


                loss = self.criterion(output.transpose(1, 2), target_sequence.long())
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        with torch.no_grad():
            ...
            next_char_probs = output.squeeze(0)[-1]


            top_k_probs, top_k_indices = torch.topk(next_char_probs, k)

            top_k_indices = [idx.item() for idx in top_k_indices]

            most_likely_chars = [char for char,idx in self.vocab.get_stoi().items() if idx in top_k_indices]
...
class NamesDataset(Dataset):
    ...
    def __init__(self, names_list, vocab):

        self.names_list = names_list
        self.vocab = vocab
        # self.max_length = max(len(name) for name in names_list)
        self.max_length = 10
...
dataset= NamesDataset(train_text,vocab)
validset= NamesDataset(validation_text,vocab)
...
# you may want to pass arguments to RNN_LM based on your implementation  # Example vocabulary size
# BEGIN CODE
embedding_dim = 128
hidden_dim = 256
dropout_prob = 0.5
num_layers=2
model = RNN_LM(vocab_size, embedding_dim, hidden_dim,num_layers,dropout_prob)
# Move the model to GPU if available
if USE_CUDA:
  model = model.cuda()
...
model_dir = 'ALOKAMGNANESWARASAI_22582/rnn'
...
# change the directory name with your SAPname and SRno
# BEGIN CODE
folder = 'ALOKAMGNANESWARASAI_22582/rnn'
